...
export async function setupFixtureRepository(
  ...
  for await (const e of glob('**/_git', { cwd: testRepoPath })) {
    const gitPlaceholderPath = join(testRepoPath, e)
    const gitPath = join(testRepoPath, dirname(e), '.git')

    if ((await lstat(gitPlaceholderPath)).isFile()) {
      // Recreate Git's indirection file so copied fixture ownership cannot
      // make the submodule look unsafe in containerized tests.
      await writeFile(gitPath, await readFile(gitPlaceholderPath))
      await rm(gitPlaceholderPath)
    } else {
      await rename(gitPlaceholderPath, gitPath)
    }
  }
