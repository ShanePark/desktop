...
> {
   ...
   *                         its git metadata) is gone. Pass `null` when the
   *                         target is the main worktree and the recorded path
   *                         should be cleared.
   */
  public async switchWorktree(
    ...
    mainWorktreePath: string | null | undefined = repository.mainWorktreePath
    ...
    const persistedMainWorktreePath =
      mainWorktreePath === null ? undefined : mainWorktreePath

    await this.db.repositories.update(repository.id, {
      path: worktreePath,
      missing,
      gitDir,
      mainWorktreePath: persistedMainWorktreePath,
    })
